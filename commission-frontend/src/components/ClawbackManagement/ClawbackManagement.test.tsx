...
describe('ClawbackManagement', () => {
  ...
  it('renders clawback management component', () => {
    render(<ClawbackManagement sales={mockSales} onCancelSale={mockOnCancelSale} />);
  ...
  it('shows active policy count', () => {
    ...
    expect(screen.getByText(/showing 2 of 2 active policies/i)).toBeInTheDocument();
  });
  ...
  it('shows placeholder message when no policy selected', () => {
    ...
    expect(screen.getByText(/select a policy to view clawback impact/i)).toBeInTheDocument();
  });
  ...
  it('shows confirmation modal when initiate clawback is clicked', () => {
    ...
    const initiateButton = screen.getByRole('button', { name: /cancel policy & initiate clawback/i });
    fireEvent.click(initiateButton);
    
    expect(screen.getAllByText('Confirm Clawback')[0]).toBeInTheDocument();
    expect(screen.getByText(/are you sure you want to cancel policy/i)).toBeInTheDocument();
  });
  ...
  it('calls onCancelSale when clawback is confirmed', async () => {
    ...
    const confirmButton = screen.getAllByRole('button', { name: /confirm clawback/i })[0];
  ...
  it('shows empty state when no policies match search', () => {
    ...
    expect(screen.getByText(/no policies match your search/i)).toBeInTheDocument();
  });
  ...
  it('shows empty state when no active policies', () => {
    ...
    render(<ClawbackManagement sales={cancelledSales} onCancelSale={mockOnCancelSale} />);
