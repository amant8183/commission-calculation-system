...
resp = requests.post(f"{BASE_URL}/agents", json={
    "name": "Mike (Director)",
    "level": 4,
    "parent_id": None
})
...
resp = requests.post(f"{BASE_URL}/bonuses/calculate", json={
    "period": period_str,
    "type": "Monthly"
})
...
if resp.status_code == 200:
    ...
    test("Monthly bonuses created", len(monthly_bonuses) > 0, 
    ...
    if sarah_bonus:
        ...
        test("Sarah's monthly bonus calculation", 
...
if resp.status_code == 200:
    ...
    test("Commissions calculated", summary.get('total_commissions_paid', 0) > 0,
...
if resp.status_code == 200:
    ...
    test("Clawbacks created", clawback_amount != 0,
...
if resp.status_code == 200:
    ...
    if cancelled_sale:
        test("Sale marked as cancelled", cancelled_sale.get('is_cancelled', False) == True)
