...
def test_create_sale_and_calculate_commissions(client, db, setup_hierarchy):
    ...
    response = client.post('/api/sales', data=json.dumps(sale_data), content_type='application/json')
    ...
    agent_comm = db.session.query(Commission).filter_by(agent_id=setup_hierarchy['agent_id']).first()
    ...
    tl_comm = db.session.query(Commission).filter_by(agent_id=setup_hierarchy['team_lead_id']).first()
    ...
    mgr_comm = db.session.query(Commission).filter_by(agent_id=setup_hierarchy['manager_id']).first()
    ...
    dir_comm = db.session.query(Commission).filter_by(agent_id=setup_hierarchy['director_id']).first()
